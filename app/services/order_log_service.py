...
class OrderLogService(BaseService):
    ...
    def add_order_latest_log(self,orders):
        for order in orders['data']:
            order_log= self.model.query.filter_by(order_id=order['id']).order_by(self.model.created_at.desc()).first()
            if order_log:
                order['order_latest_log'] = {
                    'order_log_id': order_log.id,
                    'created_at': order_log.created_at, 
                    'status':order_log.order_status,
                    'status_name': order_statuses.get_value(order_log.order_status)
                }
        return orders

    # def add_order_logs_with_this(self, items: dict) -> dict:
    #     for item in items["data"]:
    #         item["order_logs"] = self.get_order_log_by_order_id(item['id'])
    #         print(item['order_logs'])
    #     return items
    ...
    #                     'status_name': order_statuses.get_value(log.order_status)  # Add your additional key and value here
    #                 }
    #                 order_logs_dicts.update(log_dict)
